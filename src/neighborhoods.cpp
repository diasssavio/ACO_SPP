...
solution neighborhoods::swap12( solution& p_sol ) {
	unsigned n = spp.get_n();
	unsigned big_M = spp.get_big_M();
	vector < unsigned > weights = spp.get_weights();
	vector< vector < unsigned > > subsets = spp.get_subsets();
	...
	// Evaluating all possible swaps
	unsigned i_swap = 0, j_swap = 0, jj_swap = 0;
	double current_cost = p_sol.get_cost();
	for(unsigned i = 0; i < sets.size(); i++)
		for(unsigned j = 0; j < subsets.size(); j++) {
			// If j is already in the solution, skip it.
			if(find(sets.begin(), sets.end(), j + 1) != sets.end()) continue;
			for(unsigned jj = 0; jj < subsets.size(); jj++) {
				// If jj is already in the solution, skip it.
				if(find(sets.begin(), sets.end(), jj + 1) != sets.end()) continue;
				...
				for(unsigned k = 0; k < subsets[j].size(); k++)
					if(elems[ subsets[j][k] - 1 ] >= 1)
						...
						cost -= big_M;
				for(unsigned k = 0; k < subsets[jj].size(); k++)
					if(elems[ subsets[jj][k] - 1 ] >= 1)
						cost += big_M;
					else if(elems[ subsets[jj][k] - 1 ] == 0)
						cost -= big_M;
	...
	// If a better solution cost was found, make the swap.
	if(j_swap != jj_swap && i_swap != j_swap) {
		solution neighbor(p_sol);
		neighbor.remove_subset(sets[i_swap]);
		neighbor.insert_subset(j_swap + 1);
		neighbor.insert_subset(jj_swap + 1);
		#if LOGS == true
			printf("Swapped %d <-> (%d, %d)\n", sets[i_swap], j_swap + 1, jj_swap + 1);
			printf("Previous cost / New cost: %.2lf <-> %.2lf\n", p_sol.get_cost(), current_cost);
			neighbor.show_data();
		#endif
		return neighbor;
...
solution neighborhoods::swap21( solution& p_sol ) {
	unsigned n = spp.get_n();
	unsigned big_M = spp.get_big_M();
	vector < unsigned > weights = spp.get_weights();
	vector< vector < unsigned > > subsets = spp.get_subsets();
	...
	// Evaluating all possible swaps
	unsigned i_swap = 0, ii_swap = 0, j_swap = 0;
	double current_cost = p_sol.get_cost();
	for(unsigned i = 0; i < sets.size(); i++)
		for(unsigned ii = 0; ii < sets.size(); ii++)
			for(unsigned j = 0; j < subsets.size(); j++) {
				...
				// Calculating the current cost for the swap change:
				double cost = p_sol.get_cost();
				// -- Cost for removing sets[i] & sets[ii]
				cost -= weights[ sets[i] - 1 ];
				cost -= weights[ sets[ii] - 1 ];
				for(unsigned k = 0; k < subsets[ sets[i] - 1 ].size(); k++)
					if(elems[ subsets[ sets[i] - 1 ][k] - 1 ] >= 2)
						cost -= big_M;
					else if(elems[ subsets[ sets[i] - 1 ][k] - 1 ] == 1)
						cost += big_M;
				for(unsigned k = 0; k < subsets[ sets[ii] - 1 ].size(); k++)
					if(elems[ subsets[ sets[ii] - 1 ][k] - 1 ] >= 2)
						cost -= big_M;
					else if(elems[ subsets[ sets[ii] - 1 ][k] - 1 ] == 1)
						cost += big_M;
	...
	// If a better solution cost was found, make the swap.
	if(i_swap != ii_swap && i_swap != j_swap) {
		solution neighbor(p_sol);
		neighbor.remove_subset(sets[i_swap]);
		neighbor.remove_subset(sets[ii_swap]);
		neighbor.insert_subset(j_swap + 1);
		#if LOGS == true
		printf("Swapped (%d, %d) <-> %d\n", sets[i_swap], sets[ii_swap], j_swap + 1);
		printf("Previous cost / New cost: %.2lf <-> %.2lf\n", p_sol.get_cost(), current_cost);
		neighbor.show_data();
		#endif
		return neighbor;
...
solution neighborhoods::swap22( solution& p_sol ) {
	...
	// Evaluating all possible swaps
	unsigned i_swap = 0, ii_swap = 0, j_swap = 0, jj_swap = 0;
	double current_cost = p_sol.get_cost();
	for(unsigned i = 0; i < sets.size(); i++)
		for(unsigned ii = 0; ii < sets.size(); ii++)
			for(unsigned j = 0; j < subsets.size(); j++) {
	...
	// If a better solution cost was found, make the swap.
	if(i_swap != ii_swap && j_swap != jj_swap && i_swap != j_swap) {
		solution neighbor(p_sol);
		neighbor.remove_subset(sets[i_swap]);
		neighbor.remove_subset(sets[ii_swap]);
		neighbor.insert_subset(j_swap + 1);
		neighbor.insert_subset(jj_swap + 1);
		#if LOGS == true
		printf("Swapped (%d, %d) <-> (%d, %d)\n", sets[i_swap], sets[ii_swap], j_swap + 1, jj_swap + 1);
		printf("Previous cost / New cost: %.2lf <-> %.2lf\n", p_sol.get_cost(), current_cost);
		neighbor.show_data();
		#endif
		return neighbor;
...
solution& neighborhoods::execute( solution& p_sol ) {
	...
	while(i < n_neighborhoods) {
		...
		switch(NL[i]) {
			case 0:
				#if LOGS == true
					printf("Trying swap_11...\n");
				#endif
				aux = swap11(best);
				break;
			case 1:
				#if LOGS == true
					printf("Trying swap_12...\n");
				#endif
				aux = swap12(best);
				break;
			case 2:
				#if LOGS == true
					printf("Trying swap_21...\n");
				#endif
				aux = swap21(best);
				break;
			case 3:
				#if LOGS == true
					printf("Trying swap_22...\n");
				#endif
				aux = swap22(best);
				break;
		}
